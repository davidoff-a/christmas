...
function filterToys(
  bigData: IData[],
  filterField: keyof IData,
  filterQuery: string
): IData[] {
  if (filterField === "favorite") {
    return bigData.filter((item: IData) => item[filterField]);
  }
  const filteredData: IData[] = bigData.filter((item: IData) =>
...
function filterToysByNumValues(
  bigData: IData[],
  filterField: keyof IData,
  filterQuery: number[]
): IData[] {
  ...
  const filteredData = bigData.filter(
    (item: IData) =>
      Number(item[filterField]) <= max && Number(item[filterField]) >= min
  );
  localStorage.setItem("filteredArr", JSON.stringify(filteredData));
...
function minMaxValues(
  bigData: IData[],
  searchField: keyof IData,
  sortingDirection: string
): number {
  const limitValue = [...bigData].sort(
    dynamicSort(searchField, sortingDirection)
  )[0][searchField];
  return +limitValue;
}

function dynamicSort(
  sortField: keyof IData = "name",
